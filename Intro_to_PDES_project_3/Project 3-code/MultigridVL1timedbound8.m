function Uout = MultigridVL1timedbound8(Uin,Qold,f,L,k1,d,Bx,By); 
...
% if we are at the coarsest level take 10 GS iterations - should be enough
    if ((n==3)|(m==3)) 
      Uout = GSL1timedbound8(Uin,Qold,f,4,L,k1,d,Bx,By); 
    else
% otherwise begin the cycle from fine to coarsest
%
%   Start by smoothing input with 10 GS iterations - could be too many
      Usmooth = GSL1timedbound8(Uin,Qold,f,4,L,k1,d,Bx,By); 
%
%   compute the residuals 
      res  = residual1timedbound8(Usmooth,Qold,f,L,k1,d,Bx,By); 
...
%     Finally, smooth out any new high-frequency error (post-smoothing) 
      Uout = GSL1timedbound8(Usmooth,Qold,f,4,L,k1,d,Bx,By);
